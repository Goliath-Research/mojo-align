...
(MethylCall, MergeCpG, ConversionRate, Align control plane, **native Mojo
Giraffe GBZ stream map**, **native Mojo linear mapper**) run in Mojo, with
selective Python interop for GAF filtering, pack ensure, and linear prep/QC.
Set `METHYLGRAPHER_MCALL_ENGINE=python` to force Align / MethylCall /
MergeCpG / ConversionRate onto `engine.cli`.

| Component | Status |
|---|---|
| `engine/` (Python 0.2.0 port + patches) | 🟢 Complete — `python -m engine.cli`; Align backends; pack ensure + `quartet_map` **oracle**; MojoFq2bamMeth orchestration |
...
| `src/giraffe_*.mojo` + `giraffe_stream_map.mojo` | 🔵 In progress — **native Mojo** GBZ production map (GPU seed → locate → cluster → gapless → GAF); `quartet_map` oracle only — gates: Buffy ≤2 h + DS20M — `docs/GIRAFFE_SPEC.md` |
...
| `src/main.mojo` | 🟢 Complete — CLI: native Align/MethylCall/MergeCpG/ConversionRate/MojoGiraffe; MojoFq2bamMeth → `engine.fq2bam_meth` (shells `linear_mapper.mojo`); PrepareGenome/Main → `engine.cli` |
...
scripts/benchmark_clara_fq2bam_meth.sh
```
...
| MojoGiraffe (GBZ) | **Native** `giraffe_stream_map` (DeviceContext minimizers → HT locate → cluster → gapless → GAF) | Buffy-scale streaming; `quartet_map` is oracle + pack ensure only |
| MojoFq2bamMeth | Native Mojo `linear_*.mojo` (streaming batches, hash index, seeds→extend → SAM); Python convert/QC/`samtools` | Full linear map in Mojo. DeviceContext seeds both NVIDIA and AMD. Auto `bwa_fallback` unless `GPU_REQUIRE=1`. Clara/ROCm wall-clock gates before Complete |
...
engine/
  ...
  quartet_map.py        # GBZ oracle + ensure_pack / READY gate (not Align hot path)
...
src/
  ...
  giraffe_stream_map.mojo # Production GBZ hot path (native Mojo)
  giraffe_minimizer.mojo / giraffe_min_index.mojo
  giraffe_gapless.mojo / giraffe_pack.mojo / giraffe_hit.mojo
...
scripts/
  ...
  run_toy_fq2bam_meth.sh / benchmark_fq2bam_meth.sh / benchmark_clara_fq2bam_meth.sh
  build_mojo_segment_pack.py / build_mojo_gbz_cache.py
  giraffe_gaf_parity.py / smoke_mojo_stream_map.mojo
...
| `METHYLGRAPHER_GPU_REQUIRE` | empty/`1`: fail closed if DeviceContext cannot be created for nvidia/amd; explicit `1` also blocks linear BWA fallback; `0` allows host seeds |
| `METHYLGRAPHER_DUAL_GRAPH_PARALLEL` | unset: **serialize** C2T/G2A on GPU/Mojo (default); `1` overlaps (opt-in); `0` force serial |
...
| `METHYLGRAPHER_LINEAR_READ_BATCH` | linear streaming batch size (default 16384) |
| `METHYLGRAPHER_BWA_THREADS` | BWA fallback / `samtools sort` threads |
| `METHYLGRAPHER_MOJO_READ_BATCH` | streaming FASTQ batch size for `giraffe_stream_map` (default 8192) |
| `METHYLGRAPHER_PROFILE_STAGES` | stage timing prints for stream map / oracle |
