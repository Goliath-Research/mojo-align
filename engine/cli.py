#!/usr/bin/env python
# engine/cli.py — faithful port of python_reference/main.py (methylGrapher
# 0.2.0), restructured as a package entry point so it is runnable both as
# `python -m engine.cli <command> <args>` and via Python interop from the
# Mojo CLI (`src/main.mojo`), which imports `engine.cli` and calls `main()`
# directly with a Python list of argv-style strings.
#
# Patches (methylGrapher-mojo cutover, see MIGRATION_LOG.md):
...
def main(argv=None):
    """Run the methylGrapher CLI.

    ``argv`` should NOT include the program name (mirrors ``sys.argv[1:]``).
    When ``argv`` is None, ``sys.argv[1:]`` is used. Returns a process exit
    code (int) rather than calling ``sys.exit`` directly, so it is safe to
    call from Python interop callers (e.g. Mojo) as well as from the
    ``python -m engine.cli`` entry point below.
    """
    if argv is None:
    ...
    # command == "mergegaf" falls through here with no handler, matching the
    # (unimplemented) behavior of the original 0.2.0 CLI.
    return 0
