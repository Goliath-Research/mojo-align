...
# engine/mcall.py — faithful port of python_reference/mcall.py (methylGrapher 0.2.0).
# Patches (methylGrapher-mojo, see MIGRATION_LOG.md):
#   1. alignment_parse() skips malformed GAF lines with < 12 columns instead of
#      raising an IndexError deep in get_best_alignment_from_same_read_pair().
#   2. mcall_main() forces gfa_worker_num=1 unconditionally (no dual-GFA-worker
#      mode); call_parallel() asserts this invariant defensively.
...
def alignment_parse(work_dir, node_replacement_dict, minimum_identity=50, minimum_mapq=20, discard_multimapped=True, pid="0_0"):
    ...
    with _open_alignment_gaf(work_dir) as alignment_fh:
        for l in alignment_fh:
            ...
            # Patch (methylGrapher-mojo): skip malformed/truncated GAF lines.
            # A valid GAF record has at least 12 mandatory columns; anything
            # shorter cannot be a real alignment (e.g. truncated writes,
            # partial flushes) and would otherwise raise IndexError below.
            if len(l) < 12:
                continue
...
def call_parallel(
    ...
    # Patch (methylGrapher-mojo): defensively force single GFA worker
    # regardless of what the caller passed in (no dual GFA support).
    gfa_worker_num = 1
...
def mcall_main(
    ...
    # Patch (methylGrapher-mojo): always use a single GFA worker. The original
    # dual-GFA-worker mode (thread > 20) is not supported in this cutover.
    gfa_worker_num = 1
