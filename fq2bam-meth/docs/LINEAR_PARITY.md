...
The k-mer bakeoff uses **`METHYLGRAPHER_LINEAR_ENGINE=parity`**
(`src/linear_gpu_locate.mojo`). Do not retune vote/KEEP/occ there for
wall-clock. Default Align is **`fm`**. Speed work lives in
`src/linear_gpu_speed.mojo` and is documented in
[`LINEAR_ENGINES.md`](LINEAR_ENGINES.md).
