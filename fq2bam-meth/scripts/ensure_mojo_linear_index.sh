#!/usr/bin/env bash
# Prebuild Mojo linear C2T FASTA + k-mer cache beside a fleet reference FASTA.
#
# Layout (siblings of the FASTA, matching bwameth convention):
#   ${REF}.C2T.fa
#   ${REF}.mojo_linear_k${K}/{meta.txt,hits.tsv,ref.fa}
#
# Usage:
#   fq2bam-meth/scripts/ensure_mojo_linear_index.sh /path/to/ref.fasta [k]
#
# Default fleet target:
#   /work/genomes/linear/GRCh38/ensembl-114/Homo_sapiens.GRCh38.dna.primary_assembly.fa
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")/../.." && pwd)"
cd "$REPO_ROOT"
export PATH="$HOME/.pixi/bin:${PATH:-}"

DEFAULT_REF="/work/genomes/linear/GRCh38/ensembl-114/Homo_sapiens.GRCh38.dna.primary_assembly.fa"
REF="${1:-$DEFAULT_REF}"
K="${2:-${METHYLGRAPHER_LINEAR_K:-15}}"

if [[ ! -f "$REF" ]]; then
  echo "usage: $0 /path/to/ref.fasta [k]" >&2
  echo "missing: $REF" >&2
  exit 2
fi
REF="$(readlink -f "$REF")"
C2T="${REF}.C2T.fa"
CACHE="${REF}.mojo_linear_k${K}"

if [[ -f "$C2T" && -f "${CACHE}/hits.tsv" && -f "${CACHE}/meta.txt" ]]; then
  echo "Already indexed:"
  echo "  $C2T"
  echo "  $CACHE"
  ls -lh "$C2T" "${CACHE}/hits.tsv" "${CACHE}/meta.txt" "${CACHE}/ref.fa" 2>/dev/null || true
  exit 0
fi

if [[ ! -f "$C2T" ]]; then
  # Reuse a sample-local C2T if it matches this basename (avoids re-converting 3GB).
  cand="/work/samples/parabricks_sample/align.linear.mojo/work/$(basename "$REF").C2T.fa"
  if [[ -f "$cand" ]]; then
    echo "Copying existing C2T FASTA → $C2T"
    cp -a "$cand" "$C2T"
  else
    echo "Building C2T FASTA (once) → $C2T"
    python3 - <<PY
from pathlib import Path
import sys
sys.path.insert(0, "$REPO_ROOT/fq2bam-meth/python")
from fq2bam_meth import convert_fasta_c2t
convert_fasta_c2t(Path("$REF"), Path("$C2T"))
print("wrote", "$C2T")
PY
  fi
fi

if [[ -f "${CACHE}/hits.tsv" ]]; then
  echo "Cache already present: $CACHE"
  exit 0
fi

echo "Building Mojo linear k=${K} cache (slow, once) → $CACHE"
mkdir -p "$CACHE"
pixi run mojo \
  -I gpu-common/src -I fq2bam-meth/src -I giraffe/src -I methylgrapher/src \
  fq2bam-meth/src/linear_mapper.mojo \
  -ref "$C2T" \
  -cache_dir "$CACHE" \
  -k "$K" \
  -device cpu \
  -build_cache_only

[[ -f "${CACHE}/hits.tsv" ]] || { echo "ERROR: hits.tsv missing after build" >&2; exit 4; }
echo "OK:"
ls -lh "$C2T" "${CACHE}/"*
