...
# engine/mgmp.py — faithful port of python_reference/mgmp.py (methylGrapher
# 0.2.0). This module is experimental/unused scaffolding upstream (not
# imported by cli.py, mcall.py, or alignments.py); kept for reference parity.
# Patch (methylGrapher-mojo): guarded the module-level debug driver under
# `if __name__ == "__main__":` so `import engine.mgmp` no longer calls
# `sys.exit(3)` as a side effect (see MIGRATION_LOG.md).
...
if __name__ == "__main__":
    # Patch (methylGrapher-mojo): the original 0.2.0 source ran this debug
    # driver (and `sys.exit(3)`) unconditionally at module import time, which
    # made `engine.mgmp` unimportable. Guarded here so the module can be
    # imported safely; this file is dead/experimental scaffolding in upstream
    # methylGrapher (not part of the Align/MethylCall/MergeCpG call graph) and
    # is kept only for reference parity. See MIGRATION_LOG.md.
